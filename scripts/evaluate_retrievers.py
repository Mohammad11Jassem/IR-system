...
def parse_args():
    ...
    parser.add_argument(
        "--models",
        nargs="+",
        # تعديل: تم وضع الموديلات الستة المطلوبة في مشروع التخرج لتعمل تلقائياً دفعة واحدة
        default=["tfidf", "bm25", "word2vec", "bert", "serial_bm25_bert", "parallel_full"],
        help="Models to evaluate. Example: --models tfidf bm25 word2vec bert serial_bm25_bert parallel_full",
    )

    parser.add_argument("--dataset", default="main")
    
    # تعديل جوهري: تم تقييد الأعماق الافتراضية بـ 10 وثائق لتعود النتائج مطابقة لكولاب وللأصول العلمية
    parser.add_argument("--run-depth", type=int, default=10)
    parser.add_argument("--bm25-k1", type=float, default=1.2)
    parser.add_argument("--bm25-b", type=float, default=0.75)
    parser.add_argument("--precision-k", type=int, default=10)
    parser.add_argument("--ndcg-k", type=int, default=10)
    parser.add_argument("--recall-k", type=int, default=10)
    parser.add_argument("--map-depth", type=int, default=10)
    parser.add_argument("--min-relevance", type=int, default=1)

    parser.add_argument("--candidate-k", type=int, default=10)
    parser.add_argument("--per-model-k", type=int, default=10)
    parser.add_argument("--rrf-k", type=int, default=60)
    ...
    parser.add_argument("--refinement-safety-top-k", type=int, default=10)
    parser.add_argument("--refinement-safety-min-overlap-ratio", type=float, default=0.20)
    parser.add_argument("--refinement-safety-threshold", type=float, default=0.65)
    parser.add_argument("--refinement-top-history-queries", type=int, default=2)
    parser.add_argument("--refinement-max-history-terms", type=int, default=5)
    parser.add_argument("--refinement-original-term-weight", type=int, default=2)
...
def main():
    ...
    print("Summary CSV:", result["summary_csv"])
    print("Summary XLSX (Excel):", result["summary_xlsx"])
    print("Run files:")
    for model, path in result["run_paths"].items():
        print(f"  - {model}: {path}")
